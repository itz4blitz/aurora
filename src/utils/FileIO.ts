...
export class FileIO {
  ...
  static async readFile(uri: vscode.Uri): Promise<string> {
    try {
      ...
      await this.errorManager.handleError(error as Error, 'FileIO.readFile', ErrorSeverity.Error);
      throw error;
    }
  }
  ...
  static async writeFile(uri: vscode.Uri, content: string): Promise<void> {
    try {
      ...
      await this.errorManager.handleError(error as Error, 'FileIO.writeFile', ErrorSeverity.Error);
      throw error;
    }
  }
