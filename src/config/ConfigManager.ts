...
export class ConfigManager {
  ...
  async initializeConfig(): Promise<void> {
    try {
      const configuration = vscode.workspace.getConfiguration('aurora');
      const storedModel = configuration.get<AIModel>('defaultModel');

      if (storedModel) {
        this.currentModel = storedModel;
        this.config.defaultModel = storedModel;
      }

      const storedConfigs = configuration.get<ModelConfigs>('modelConfigs');
      if (storedConfigs) {
      ...
      if (geminiKey) {
        const isValid = await this.secretStorage.validateGeminiKey();
        if (!isValid) {
  ...
  async hasValidGeminiKey(): Promise<boolean> {
    try {
      return await this.secretStorage.validateGeminiKey();
    } catch (error) {
  ...
  async hasValidOpenAIKey(): Promise<boolean> {
    try {
      return await this.secretStorage.validateOpenAIKey();
    } catch (error) {
  ...
  async hasValidAnthropicKey(): Promise<boolean> {
    try {
      return await this.secretStorage.validateAnthropicKey();
    } catch (error) {
