...
export class ModelManager {
  ...
  async generateResponse(
    ...
    try {
      ...
      const isModelValid = await this.validateModel(selectedModel);
      if (!isModelValid) {
        throw new Error(`Model ${String(selectedModel)} is not properly configured`);
      }
      ...
      switch (selectedModel) {
        ...
        default:
          throw new Error(`Unsupported model: ${String(selectedModel)}`);
      }
    } catch (error) {
  ...
  async validateModel(model: AIModel): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error validating model: ${String(model)}`, error as Error);
      return false;
    }
  }
  ...
  async switchModel(model: AIModel): Promise<boolean> {
    try {
      if (await this.validateModel(model)) {
        ...
        this.logger.info(`Successfully switched to model: ${String(model)}`);
        return true;
      }
      return false;
    } catch (error) {
  ...
  getModelCapabilities(model: AIModel): Record<string, boolean> {
