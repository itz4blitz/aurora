...
export class CodeBlockRenderer {
    ...
    private constructor() {
        ...
        this.supportedLanguages = new Set([
            ...
            'html', 'css', 'json', 'yaml', 'markdown', 'sql', 'shell'
        ]);
    }
    ...
    private async createTempDocument(content: string, language: string): Promise<vscode.TextDocument | null> {
        try {
    ...
    private async tokenize(document: vscode.TextDocument): Promise<vscode.SemanticTokens> {
        ...
        return await vscode.commands.executeCommand(
            '_executeDocumentSemanticTokens',
            document.uri
        );
    }
    ...
    private convertTokensToHTML(tokens: vscode.SemanticTokens, originalCode: string): string {
        ...
        for (let i = 0; i < tokenData.length; i += 5) { // Semantic tokens data comes in groups of 5
