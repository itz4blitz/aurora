...
export function registerUI(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('aurora.showCodeSuggestion', async (code: string, language: string) => {
            await uiManager.showCodeSuggestion(code, language);
        }),

        vscode.commands.registerCommand('aurora.showDiff', async (original: string, modified: string) => {
